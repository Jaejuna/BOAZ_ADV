...
if __name__ == '__main__':
    ...
    map_pcd = getMapPointCloud(client, args.voxel_size) # 맵의 포인트 클라우드를 얻음

    replay = deque(maxlen=args.mem_size) 
    losses = []

    for epoch in range(args.epochs):
        client.reset() # 현재의 시뮬레이션 상태를 초기화하고 에어드론을 리스폰스 시킴.
        if args["drone"].set_random_pose:
            setRandomPose(client, args) # 드론의 위치를 랜덤으로 설정

        pcd_global = o3d.geometry.PointCloud() # 빈 3D 포인트 클라우드 생성, 이후에 드론의 위치를 기준으로 포인트 클라우드를 추가하여 채움
        curr_pcd = mergePointClouds(pcd_global, getPointCloudByIntrinsic(client), client) # 현재 포인트 클라우드를 전체 클라우드에 병합, 현재시점의 데이터를 전체 데이터에 누적
        pcd_global = copy.deepcopy(curr_pcd) # 현재 포인트 클라우드를 전체 클라우드에 복사
        if args.live_visualization:
            putDataIntoQueue(data_queue, pcd_global) # 3D 포인트 클라우드 데이터를 큐에 넣음
        ...
        while(status == "running"):     # 상태가 running일때 반복
            ...
            client.moveToPositionAsync(x, y, z, args["drone"].default_velocity,  # 드론을 x, y, z 위치로 이동시킴, velocity는 드론의 속도
                                    drivetrain=airsim.DrivetrainType.MaxDegreeOfFreedom,    # 드론의 운행 방식을 설정, MaxDegreeOfFreedom는 자유도가 가장 높은 운행 방식 
                                    yaw_mode=airsim.YawMode(False, clacDuration(args["drone"].yaw_rate, radian))).join()    # 드론의 회전(YAW) 모드를 설정. 이동작업중에는 회전하지 않으므로 False
                                                                                                                            # clacDuration 함수는 회전 속도를 설정하는 함수
            ...
            curr_pcd = mergePointClouds(pcd_global, getPointCloudByIntrinsic(client), client)   # 현재 포인트 클라우드를 전체 클라우드에 병합, 현재시점의 데이터를 전체 데이터에 누적
            reward = calcReward(map_pcd, pcd_global, curr_pcd, client, running_time, args)   # 보상을 계산, 맵의 포인트 클라우드, 전체 포인트 클라우드, 현재 포인트 클라우드, 드론이 움직인 시간, args를 인자로 전달
            pcd_global = copy.deepcopy(curr_pcd)    # 현재 포인트 클라우드를 전체 클라우드에 복사
            if args.live_visualization:
                putDataIntoQueue(data_queue, pcd_global)    # 3D 포인트 클라우드 데이터를 큐에 넣음
        ...
        if epoch % args.eval_freq == 0 and epoch != 0: # 일정 주기마다 모델을 평가
            client.reset()  # 현재의 시뮬레이션 상태를 초기화하고 에어드론을 초기상태로 리셋
            acc = getAccuracy(model1, client, map_pcd, args)    # 모델의 정확도를 계산
